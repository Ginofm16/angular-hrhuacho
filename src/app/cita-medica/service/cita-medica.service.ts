...
export class CitaMedicaService {
  ...
  private urlEndPoint: string = 'http://localhost:8080/api/citaPaciente';
...
update(citaMedica: CitaMedica):Observable<any>{
  ...
  console.log(citaMedica);
  return this.http.put<any>(`${this.urlEndPoint}/${citaMedica.pro_codigo}`,citaMedica).pipe(
      catchError(e =>{
          if(e.status==400){
              return throwError(e);
          }
...
getProgramaciones(): Observable<Programacion[]>{
  ...
  return this.http.get<Programacion[]>(this.urlEndPoint+'/programacion');
